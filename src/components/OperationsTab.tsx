...
import { formatVNDate, formatVNTime, parseDate } from '../utils/time';

import { getModuleForTemplateType } from '../utils/formModules';
import { canEditModule } from '../services/permissionService';
import { canDeleteDocument } from '../services/ownershipService';
...
export function OperationsTab() {
  ...
  const canEdit = currentUser?.role ? canEditModule(currentUser.role, 'OPERATIONS') : false;
          ...
          {displayedForms.length > 0 ? (
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {displayedForms.map((form) => (
                  ...
                  >
                    {canEdit && canDeleteDocument(currentUser, form, 'OPERATIONS') && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          setDeleteRequest({
                    ...
                    <div className="text-xs text-stone-600 border-t border-stone-200 pt-3 mt-1 space-y-1.5">
                      <div className="flex items-center justify-between">
                        <span className="text-stone-500 font-medium">
                          {form.templateType === 'TASK_PLAN' ? 'Ngày tạo kế hoạch:' : 'Ngày tạo báo cáo:'}
                        </span>
                        <span className="font-bold text-emerald-800">
                          {formatVNDate(form.createdAt) || (form.formData?.createdDate ? formatVNDate(form.formData.createdDate) : '') || '—'}
                        </span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-stone-500 font-medium">Người lập:</span>
                        <span className="font-semibold text-stone-700">{usersMap[form.createdBy] || form.createdByName || form.createdBy || 'Không rõ'}</span>
                      </div>
                    </div>
                  </div>
                ))}
