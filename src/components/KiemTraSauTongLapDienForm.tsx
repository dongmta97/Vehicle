...
export const KiemTraSauTongLapDienForm: React.FC<Props> = ({ vehicle, existingFormId, targetSessionId, templateName, stageName, templateType, initialData, onSaved, onClose }) => {
        ...
        >
          ...
          <div className="flex justify-between items-end mb-6 font-times text-[15px]">
            <div className="flex-1 flex mr-4">
              ...
              <input 
                type="text" 
                value={typeof (formData.cumCongDoan !== undefined ? formData.cumCongDoan : 'Kiểm tra sau tổng lắp hệ thống điện') === 'string' ? (formData.cumCongDoan !== undefined ? formData.cumCongDoan : 'Kiểm tra sau tổng lắp hệ thống điện').normalize('NFC') : (formData.cumCongDoan !== undefined ? formData.cumCongDoan : 'Kiểm tra sau tổng lắp hệ thống điện')}
          ...
          <div className="mb-4">
            ...
            <AutoResizeTextarea
              value={typeof (formData.ketLuan !== undefined ? formData.ketLuan : (formData.conclusion || 'Cụm- công đoạn đã được kiểm tra đúng Quy trình công nghệ.')) === 'string' ? (formData.ketLuan !== undefined ? formData.ketLuan : (formData.conclusion || 'Cụm- công đoạn đã được kiểm tra đúng Quy trình công nghệ.')).normalize('NFC') : (formData.ketLuan !== undefined ? formData.ketLuan : (formData.conclusion || 'Cụm- công đoạn đã được kiểm tra đúng Quy trình công nghệ.'))}
