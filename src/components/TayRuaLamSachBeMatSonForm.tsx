...
const ITEMS: any[] = [
  { category: 'I. KIỂM TRA LÀM SẠCH', stt: 1, noiDung: 'Kiểm tra đồng: Xe phải cứng chắc, mối hàn phải phẳng, ngấu đều.', yeuCau: '- Vỏ xe không bập bùng; - Không có rỗ mọt trên toàn xe; - Mài phẳng mối hàn.' },
  { category: 'I. KIỂM TRA LÀM SẠCH', stt: 2, noiDung: 'Làm sạch xe.', yeuCau: '- Làm sạch đất, dầu mơ; - Làm sạch xỉ hàn.' },
  { category: 'I. KIỂM TRA LÀM SẠCH', stt: 3, noiDung: 'Kiểm tra hình dáng : Hình dạng xe phải hài hoa, cân đối.', yeuCau: '- Độ không đồng phẳng ≤1,5mm; - Theo mẫu xe nguyên thuỷ mà TCN qui định.' },
...
export const TayRuaLamSachBeMatSonForm: React.FC<Props> = ({ vehicle, existingFormId, targetSessionId, templateName, stageName, templateType, initialData, onSaved, onClose }) => {
        ...
        >
          ...
          <div className="flex justify-between items-end mb-6 font-times text-[15px]">
            <div className="flex-1 flex mr-4">
              ...
              <input 
                type="text" 
                value={typeof (formData.cumCongDoan !== undefined ? formData.cumCongDoan : 'Tẩy rửa, làm sạch bề mặt sơn') === 'string' ? (formData.cumCongDoan !== undefined ? formData.cumCongDoan : 'Tẩy rửa, làm sạch bề mặt sơn').normalize('NFC') : (formData.cumCongDoan !== undefined ? formData.cumCongDoan : 'Tẩy rửa, làm sạch bề mặt sơn')}
            ...
            <div className="flex gap-4">
              ...
              <div className="flex">
                ...
                <input 
                  type="text" 
                  value={typeof (formData.soTo !== undefined ? formData.soTo : '1') === 'string' ? (formData.soTo !== undefined ? formData.soTo : '1').normalize('NFC') : (formData.soTo !== undefined ? formData.soTo : '1')}
          ...
          <div className="mb-4">
            ...
            <AutoResizeTextarea
              value={typeof (formData.ketLuan !== undefined ? formData.ketLuan : (formData.conclusion || 'Thân, vỏ xe được tẩy rửa làm sạch đúng Quy trình công nghệ.')) === 'string' ? (formData.ketLuan !== undefined ? formData.ketLuan : (formData.conclusion || 'Thân, vỏ xe được tẩy rửa làm sạch đúng Quy trình công nghệ.')).normalize('NFC') : (formData.ketLuan !== undefined ? formData.ketLuan : (formData.conclusion || 'Thân, vỏ xe được tẩy rửa làm sạch đúng Quy trình công nghệ.'))}
