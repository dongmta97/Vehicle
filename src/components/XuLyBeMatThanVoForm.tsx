...
export const XuLyBeMatThanVoForm: React.FC<Props> = ({ vehicle, existingFormId, targetSessionId, templateName, stageName, templateType, initialData, onSaved, onClose }) => {
        ...
        >
          ...
          <div className="flex justify-between items-end mb-6 font-times text-[15px]">
            <div className="flex-1 flex mr-4">
              ...
              <input 
                type="text" 
                value={typeof (formData.cumCongDoan !== undefined ? formData.cumCongDoan : 'Xử lý bề mặt thân, vỏ xe') === 'string' ? (formData.cumCongDoan !== undefined ? formData.cumCongDoan : 'Xử lý bề mặt thân, vỏ xe').normalize('NFC') : (formData.cumCongDoan !== undefined ? formData.cumCongDoan : 'Xử lý bề mặt thân, vỏ xe')}
          ...
          <div className="mb-4">
            ...
            <AutoResizeTextarea
              value={typeof (formData.ketLuan !== undefined ? formData.ketLuan : (formData.conclusion || 'Thân, vỏ xe được xử lý bề mặt đúng Quy trình công nghệ.')) === 'string' ? (formData.ketLuan !== undefined ? formData.ketLuan : (formData.conclusion || 'Thân, vỏ xe được xử lý bề mặt đúng Quy trình công nghệ.')).normalize('NFC') : (formData.ketLuan !== undefined ? formData.ketLuan : (formData.conclusion || 'Thân, vỏ xe được xử lý bề mặt đúng Quy trình công nghệ.'))}
