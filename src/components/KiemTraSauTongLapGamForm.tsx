...
export const KiemTraSauTongLapGamForm: React.FC<Props> = ({ vehicle, existingFormId, targetSessionId, templateName, stageName, templateType, initialData, onSaved, onClose }) => {
        ...
        >
          ...
          <div className="flex justify-between items-end mb-6 font-times text-[15px]">
            <div className="flex-1 flex mr-4">
              ...
              <input 
                type="text" 
                value={typeof (formData.cumCongDoan !== undefined ? formData.cumCongDoan : 'Kiểm tra sau tổng lắp Gầm') === 'string' ? (formData.cumCongDoan !== undefined ? formData.cumCongDoan : 'Kiểm tra sau tổng lắp Gầm').normalize('NFC') : (formData.cumCongDoan !== undefined ? formData.cumCongDoan : 'Kiểm tra sau tổng lắp Gầm')}
