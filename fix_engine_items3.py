...
# Replace actual newlines in string literals with \\n, using content.replace is safer for JS files.
new_items = """const DEFAULT_INSPECTION_ITEMS = [
  ...
  { id: 11, content: "Hệ thống phân phối khí", unit: "", requirement: "Đúng chủng loại, hoạt động ổn định, tin cậy" },
  { id: 12, content: "Tình trạng các doăng phớt, chảy dầu", unit: "", requirement: "Không bị chảy dầu" }
];"""

# Wait, since the file currently is broken with multiline string, let's fix it by regex matching the whole block again.
content = re.sub(r'const DEFAULT_INSPECTION_ITEMS = \[.*?\];', new_items.replace('\\', '\\\\'), content, flags=re.DOTALL)

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)
print("✅ Đã cập nhật DEFAULT_INSPECTION_ITEMS")
